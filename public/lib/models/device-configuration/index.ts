...
import * as exchangeEntitySchema
  from '@fastybird/metadata/resources/schemas/modules/devices-module/entity.device.configuration.json'
import {
  ModuleOrigin,
...
const moduleActions: ActionTree<DeviceConfigurationState, unknown> = {
  ...
  async transmitData(_store, payload: { configuration: DeviceConfigurationInterface, value: string }): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      DeviceConfiguration.wamp().call<{ data: string }>({
        routing_key: ActionRoutes.DEVICE_CONFIGURATION,
  ...
  async socketData({ state, commit }, payload: { origin: string, routingKey: string, data: string }): Promise<boolean> {
    ...
    if (
      ![
        RoutingKeys.DEVICES_CONFIGURATION_ENTITY_REPORTED,
    ...
    if (validate(body)) {
      if (
        ...
        payload.routingKey === RoutingKeys.DEVICES_CONFIGURATION_ENTITY_DELETED
      ) {
        return true
      }
      ...
      if (payload.routingKey === RoutingKeys.DEVICES_CONFIGURATION_ENTITY_DELETED) {
        ...
        commit('SET_SEMAPHORE', {
          type: payload.routingKey === RoutingKeys.DEVICES_CONFIGURATION_ENTITY_REPORTED ? SemaphoreTypes.GETTING : (payload.routingKey === RoutingKeys.DEVICES_CONFIGURATION_ENTITY_UPDATED ? SemaphoreTypes.UPDATING : SemaphoreTypes.CREATING),
          id: body.id,
        })
