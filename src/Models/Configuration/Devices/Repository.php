	...
	): MetadataDocuments\DevicesModule\Device|null
	{
		try {
			$document = $this->cache->load(
				...
				function () use ($queryObject, $type, &$dependencies): MetadataDocuments\DevicesModule\Device|null {
		...
		try {
			$documents = $this->cache->load(
				...
				function () use ($queryObject, $type, &$dependencies): array {
					...
					return array_map(
						fn (stdClass $item): MetadataDocuments\DevicesModule\Device => $this->entityFactory->create(
							$type,
							$item,
						),
						$result,
